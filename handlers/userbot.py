...
from config import BOT_USERNAME, BOT_OWNER, cp_version, BawsFM_version
from callsmusic.callsmusic import client as BawsFM
from handlers.ownerstuff import _check_heroku

# To Block a PM'ed User
@BawsFM.on_message(filters.private & filters.command("block", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  shit_id = message.chat.id
  gonna_block_u = await message.edit_text("`Blocking User...`")
  try:
    await BawsFM.block_user(shit_id)
...
@BawsFM.on_message(filters.command("unblock", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await message.edit_text("`Unblocking User...`")
  try:
    await BawsFM.unblock_user(good_bro)
...
@BawsFM.on_message(filters.private & filters.command("chats", [".", "/"]) & filters.me & ~filters.edited)
async def ubgetchats(_, message: Message):
  getting_chats = await message.edit_text("`Checking Your Chats, Hang On...`")
  async for dialog in BawsFM.iter_dialogs():
    try:
      total = await BawsFM.get_dialogs_count()
...
# Leave From a Chat
@BawsFM.on_message(filters.command("kickme", [".", "/"]) & filters.me & ~filters.edited)
async def ubkickme(_, message: Message):
  i_go_away = await message.edit_text("`Leaving This Chat...`")
  try:
    await BawsFM.leave_chat(message.chat.id)
...
@BawsFM.on_message(filters.private & filters.command("pmlogs", [".", "/"]) & filters.me & ~filters.edited)
@_check_heroku
async def getlogs(client: BawsFM, message: Message, app_):
  ...
  if status == "on":
    ...
    chat_pic = "cache/BawsFM.jpg"
    try:
      await logmsg.edit("`Creating Private Group Now...`!")
      pmchat = await BawsFM.create_group(f"Nexa Userbot's PM Logs", BOT_OWNER)
      chat_id = pmchat.id
      await BawsFM.set_chat_photo(chat_id=chat_id, photo=chat_pic)
      await logmsg.edit(f"`Successfully Finished Step 1, To Enable This Feature Please Check Your Log Group That Created Now!!` \n\n ~ **@NexaBotsUpdates**")
      await client.send_message(chat_id, f"**Welcome to @{(await BawsFM.get_me()).username}'s PM Log Group!** \nThis Chat will Contain All PM Messages Of **@{(await BawsFM.get_me()).username}** ! \n\n\n`/setvar PM_LOG_CHAT_ID {chat_id}` \n\n ✪ **Please Copy and Send Above Command To Your @{BOT_USERNAME} Now**!")
    except Exception as lol:
    ...
    try:
      ...
      await BawsFM.leave_chat(PM_LOG_CHAT_ID, delete=True)
...
@BawsFM.on_message(filters.private)
async def sendpmlol(client: BawsFM, message: Message):
  BawsFM_ID = int((await BawsFM.get_me()).id)
  if message.from_user.id == BOT_OWNER or message.from_user.id == BawsFM_ID:
    return
  pmlogchat = PM_LOG_CHAT_ID
  userinfo = await BawsFM.get_users(user_ids=message.from_user.id)
